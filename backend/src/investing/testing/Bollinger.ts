{
    ...
    // 백테스트 함수
    function backtestBollingerBands(
        data: DataPoint[],
    ...
    const n = 20; // 이동 평균 기간
    const k = 2; // 표준 편차 계수
    calculateBollingerBands(data, n, k);
    const bollingerBandsResult = backtestBollingerBands(data, initialCapital);

    const finalCapitalBollingerBands =
        bollingerBandsResult[bollingerBandsResult.length - 1].capital;
    const returnRateBollingerBands =
        (finalCapitalBollingerBands! / initialCapital - 1) * 100;

    console.log("Bollinger Bands Strategy Results:");
    console.log(bollingerBandsResult.slice(-10));
    console.log(`Final Capital: ${finalCapitalBollingerBands}`);
    console.log(`Return Rate: ${returnRateBollingerBands.toFixed(2)}%`);

    // Final Capital: 8910.016977928693
    // Return Rate: -10.90%
}
