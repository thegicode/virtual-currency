{
    ...
    // 이동 평균 교차와 MACD를 이용한 추세 추종 전략
    function calculateTrendFollowingStrategy(
        data: DataPoint[],
        shortPeriod: number,
        longPeriod: number,
        signalPeriod: number
    ...
    // 백테스트 함수
    function backtestTrendFollowingStrategy(
        data: DataPoint[],
    ...
    // 이동 평균 교차와 MACD를 이용한 추세 추종 전략 계산 및 백테스트
    const initialCapital = 10000; // 초기 자본
    const shortPeriod = 12; // 단기 이동 평균 기간
    const longPeriod = 26; // 장기 이동 평균 기간
    const signalPeriod = 9; // MACD 신호 기간

    calculateTrendFollowingStrategy(
        data,
        shortPeriod,
        longPeriod,
        signalPeriod
    );
    const { data: trendFollowingResult, tradeCount } =
        backtestTrendFollowingStrategy(data, initialCapital);

    const finalCapitalTrendFollowing =
        trendFollowingResult[trendFollowingResult.length - 1].capital;
    const returnRateTrendFollowing =
        (finalCapitalTrendFollowing! / initialCapital - 1) * 100;
    ...
    //     Final Capital: 10934.100246507805
    // Return Rate: 9.34%
    // Trade Count: 2
}
