...
export default class BacktestTable extends HTMLElement {
    ...
    public render(data: IBacktest9[], index: number) {
        this.data = data;
    ...
    private crateTable() {
        ...
        this.data
            .map((aData: IBacktest9, index) => this.createItem(aData, index))
    ...
    private createItem(aData: IBacktest9, index: number) {
        const cloned = cloneTemplate<HTMLElement>(this.itemTemplate);

        const parseData = {
            index,
            date: aData.date.slice(0, 10),
            openingPrice: aData.openingPrice?.toLocaleString(),
            volume: aData.volume?.toLocaleString(),
